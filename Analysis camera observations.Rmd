...
#The data base consists of consecutive shots made to individuals that were feeding on 
#Heliconia tortuosa, so there might be multiple shots to the same individual. I would like to tag
#with a unique id a same individual that was observed during multiple shots. 
...
#I consider different individuals when the difference is greater than 100 seconds and less than 0 
#(negative values indicate difference in an observation in the evening and in the morning). 
...
base_limpia_2<-data.frame(unique(base_limpia$unique_id))

#Tag each individual with its scientific name
base_limpia_2$id<-substr(base_limpia_2$unique.base_limpia.unique_id.,1,4)
base_limpia_2$complete_name<-""
base_limpia_2$complete_name<-ifelse(base_limpia_2$id=="blac","Myadestes melanops","")
base_limpia_2$complete_name<-ifelse(base_limpia_2$id=="Blue","Momotus momota",base_limpia_2$complete_name)
base_limpia_2$complete_name<-ifelse(base_limpia_2$id=="Clay","Turdus grayi",base_limpia_2$complete_name)
base_limpia_2$complete_name<-ifelse(base_limpia_2$id=="Oran","Catharus aurantiirostris",base_limpia_2$complete_name)

#Create a row with 1's to aggregate and obtain number of visits per species
base_limpia_2$visits<-1
base_limpia_2<-aggregate(base_limpia_2$visits,by=list(base_limpia_2$complete_name),FUN=sum)

colnames(base_limpia_2)<-c("species","visits_eats")
...
shak_base<-read.xlsx("Shakil data final.xlsx",sheetIndex=1)
...
visits_eats<-rbind(base_limpia_2,visits_eats)
